...
alias glog='git log --graph --pretty="%C(yellow)%h %C(red)%cd %Creset%s %C(green)%an" --date=relative'
alias gtag='git log --pretty="%C(yellow)%h %Creset%s%C(green)%d"'
...
alias shuf="ruby -e 'puts STDIN.readlines.shuffle'"
alias helpme="/Users/roop/src/me/helpme/helpme"
alias dhcpbounce="sudo ipconfig set $(networksetup -listallhardwareports | grep Wi-Fi -A1 | tail -1 | cut -c 9-) DHCP"
